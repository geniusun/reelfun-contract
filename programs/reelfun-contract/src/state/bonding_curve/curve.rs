...
impl BondingCurve {
    ...
    pub fn calculate_fee(&self, amount: u64, time_now: i64) -> Result<u64> {
        ...
        // time_diff is in seconds; Solana slot ≈ 0.4s, so slots ≈ time_diff * 1000 / 400
        let time_diff = time_now - start_time;
        let slots_passed = time_diff.checked_mul(1000)
            .unwrap_or(i64::MAX)
            .checked_div(400)
            .unwrap_or(i64::MAX);
        msg!("Time diff: {} s ({} slots)", time_diff, slots_passed);

        let sol_fee: u64;

        if slots_passed < 150 {
            // Phase 1: 99% fees in first ~60 seconds (150 slots)
            msg!("Phase 1: 99% fees between slot 0 - 150");
            sol_fee = bps_mul(9900, amount, 10_000).unwrap();
        } else if slots_passed <= 250 {
            // Phase 2: Linear decrease from 99% to 1% between slot 150-250 (~60-100s)
            ...
            sol_fee = bps_mul(fee_bps.max(100) as u64, amount, 10_000).unwrap();
        } else {
            // Phase 3: 1% fees after ~100 seconds
            msg!("Phase 3: 1% fees after 250 slots");
    ...
    pub fn apply_sell(&mut self, token_amount: u64) -> Option<SellResult> {
        ...
        let mut sol_amount = self.get_sol_for_sell_tokens(token_amount)?;
        msg!("apply_sell: sol_amount: {}", sol_amount);

        // Clamp sol_amount to real_sol_reserves (same as pump.fun)
        // This handles edge cases where accumulated rounding causes
        // sol_amount to slightly exceed real_sol_reserves after multiple trades
        if sol_amount > self.real_sol_reserves {
            msg!("apply_sell: clamping sol_amount {} to real_sol_reserves {}", sol_amount, self.real_sol_reserves);
            sol_amount = self.real_sol_reserves;
        }
        ...
        // New Real Sol Reserves (safe after clamp)
    ...
    pub fn get_tokens_for_buy_sol(&self, sol_amount: u64) -> Option<u64> {
        ...
        // Full precision: k = virtual_sol * virtual_token (no intermediate truncation)
